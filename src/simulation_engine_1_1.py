...
Discrete event simulation engine for the BPI 2017 loan application process.
Supports three allocation methods: standard heuristics, Park & Song (2019), and deep RL.
"""
...
# process completions before checks, arrivals last
_EVENT_PRIORITY = {
...
@dataclass
class Task:
    """Task waiting in the queue. pending_schedule means we defer the schedule log until we know it actually has to wait."""
    uid: str
    case_id: str
    activity: str
...
class CaseAttributeSampler:
    """Samples case attributes (LoanGoal, ApplicationType, Amount) from historical distributions."""
    ...
    def _sample_amount_logical(self, loan_goal: str, app_type: str) -> float:
        """Sample amount using most specific match available, fall back to global."""
        # Try most specific
...
class SimulationEngine:
    def __init__(
        ...
        *,
        ...
        """Set up all sub-models and simulation parameters."""
    ...
    def _enqueue_waiting_no_log(self, t: pd.Timestamp, case_id: str, activity: str) -> Task:
        # schedule event is deferred until we know the task actually has to wait
        t = pd.Timestamp(t).tz_convert("UTC")
        task = Task(
    ...
    def _flush_schedules(self, t: pd.Timestamp):
        """Log schedule events for tasks that couldn't start immediately."""
        t = pd.Timestamp(t).tz_convert("UTC")
    ...
    def _resource_check(self, t: pd.Timestamp):
        """Suspend tasks whose resource is leaving, and reschedule those that can now finish."""
        t = pd.Timestamp(t).tz_convert("UTC")
    ...
    def _allocate(self, t: pd.Timestamp):
        """Assign free resources to waiting/suspended tasks. Suspended queue has priority."""
        t = pd.Timestamp(t).tz_convert("UTC")
        ...
        while True:
            ...
            def try_queue(queue: Deque[Task], resumed: bool) -> bool:
                ...
                for _ in range(n):
                    ...
                    # check if any candidate can finish within the current bucket
    ...
    def _allocate_batch(self, t: pd.Timestamp):
        """Accumulate batch_size tasks then dispatch all at once using the cost-matrix solver."""
    ...
    def _predict_upcoming_tasks(self, t: pd.Timestamp) -> List[Dict]:
        """Predict the next activity for each running case, based on remaining processing time."""
    ...
    def _allocate_park_song(self, t: pd.Timestamp):
        """Park & Song (2019) allocation: include predicted tasks in cost matrix to enable strategic idling."""
    ...
    def _rl_get_state(self, t: pd.Timestamp) -> np.ndarray:
        """Build the RL observation vector: [busy×|R|, remaining_norm×|R|, task_count×|A|]."""
    ...
    def _rl_get_action_mask(self, t: pd.Timestamp) -> np.ndarray:
        """Return valid-action mask for MaskablePPO. Postpone action is always valid."""
        nR = len(self._all_resources_list)
        nA = len(self._all_activities_list)
    ...
    def _allocate_rl(self, t: pd.Timestamp):
        """Run one inference step from the attached RL policy."""
    ...
    def _init_run(self, max_cases: Optional[int] = None) -> None:
        """Seed the event queue for step-by-step execution (RL environment uses this)."""
    ...
    def _step_one_event(self) -> Optional[Tuple[str, pd.Timestamp]]:
        """Process one event. Returns (event_type, timestamp) or None at end of simulation."""
        ev = self._pop_event()
        if ev is None:
            return None
    ...
    def _export_xes(self, df: pd.DataFrame, xes_path: str):
        if pm4py is None:
            print("pm4py not installed, skipping XES export")
            return
